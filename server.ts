...
async function getPgClient() {
  if (!pgClient) {
    ...
    console.log("[Server] Connected to Vercel Postgres database via direct connection.");
  }
  return pgClient;
}
...
try {
  ...
  if (fs.existsSync(firebaseConfigPath)) {
    ...
    if (isPlaceholder) {
      console.log("[Server] Placeholder or incomplete Firebase configuration detected. Skipping Firestore; operating in fallback Local lowdb mode.");
    } else {
